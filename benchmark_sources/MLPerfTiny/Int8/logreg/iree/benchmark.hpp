...
#include "logreg_int8_input_data.h"
#include "logreg_int8_model_settings.h"
...
class Benchmark {
  ...
  public:
    ...
    inline int run_benchmark() {
        ...
        for (size_t i = 0; i < logreg_int8_input_rank; ++i) {
            shape[i] = (iree_hal_dim_t)logreg_int8_input_shape[i];
        }

        iree_hal_buffer_view_t* arg0 = nullptr;
        iree_status_t status = iree_hal_buffer_view_allocate_buffer_copy(
            iree_runtime_session_device(session), device_allocator, logreg_int8_input_rank, shape,
            ...
            iree_make_const_byte_span(logreg_int8_input_data[0], logreg_int8_input_data_len[0]), &arg0);
        ...
        if (iree_status_is_ok(iree_hal_buffer_map_range(
                ...
                0, logreg_int8_output_data_ref_len, &mapping))) {
            const int8_t* out_ptr = (const int8_t*)mapping.contents.data;
            for (size_t j = 0; j < logreg_int8_output_data_ref_len; ++j) {
                if (abs((int)out_ptr[j] - (int)logreg_int8_output_data_ref[j]) > 1) {
